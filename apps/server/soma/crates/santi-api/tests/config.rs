...
use mini_stim_server_soma::config::{
    AppCommand, ChatCompletionsConfig, ConfigService, ProviderConfig,
};
...
fn parses_export_openapi_command() {
    let service = ConfigService::from_args(args(["mini-stim-server-soma", "export-openapi"]))
        .expect("config service");
...
fn write_config(content: &str) -> PathBuf {
    let id = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    let path = std::env::temp_dir().join(format!("mini-stim-config-{id}.toml"));
